...
// 动态导入组件，减少初始加载体积
const FormSection = dynamic(() => import("@/components/memeverse/form-section").then((mod) => mod.FormSection), {
  ssr: false,
  loading: () => <div className="animate-pulse bg-purple-900/20 rounded-xl h-[400px]"></div>,
})

const ToggleSwitch = dynamic(() => import("@/components/memeverse/toggle-switch").then((mod) => mod.ToggleSwitch), {
  ssr: false,
})

const NetworkModal = dynamic(() => import("@/components/memeverse/network-modal").then((mod) => mod.NetworkModal), {
  ssr: false,
})

const FundTypeModal = dynamic(() => import("@/components/memeverse/fund-type-modal").then((mod) => mod.FundTypeModal), {
  ssr: false,
})

const OmniChainsModal = dynamic(
  () => import("@/components/memeverse/omni-chains-modal").then((mod) => mod.OmniChainsModal),
  {
    ssr: false,
  },
)

const SubmitButton = dynamic(() => import("@/components/memeverse/submit-button").then((mod) => mod.SubmitButton), {
  ssr: false,
})
